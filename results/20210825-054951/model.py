...
def get_density(mus, sigmas, points):
    ...
    :return: [batch_size, ray_batch_size, M_S]. Volume densities.
    """
    ...
    densities = tf.reduce_sum(tf.squeeze(densities, [4, 5]), axis=-1)
...
def numerical_interg(sigmas, fs, M_S=M_S, ray_len=2., include_feature=True):
    ...
    :param sigmas: [batch_size, ray_batch_size, M_S]. Volume densities.
    ...
    :return: [batch_size, ray_batch_size, M_F]. Final features.
            or [batch_size, ray_batch_size, 1]. Final densities.
    """
    ...
    ts = tf.stack(output_list, axis=-1)
...
class Model(GANModelDesc):
    ...
    def build_graph(self, img, box, mask, bbx, z, z2):
        ...
        with argscope([Conv2D, Conv2DTranspose, tf.layers.conv2d]):
            ...
            with tf.variable_scope('gen'):
                ...
                with tf.variable_scope('gen_img'):
                    ...
                    gen_img = self.neural_renderer(feature_img)
                    ...
                    feature_img_rot = self.volume_renderer(mus_rot, sigmas_rot, z_f, roty=randomrot,
                                                           M_S=M_S)
                    gen_img_rot = self.neural_renderer(feature_img_rot)
        ...
        with tf.name_scope('losses'):
            with tf.name_scope('mask_losses'):
                ...
                with tf.name_scope('m_recon_loss'):
                    mask_recon_loss = tf.reduce_mean(tf.abs(img - gen_img), name='mask_recon_loss')
        ...
        self.g_loss = G_loss_img + LAMBDA * FM_loss_mask + LAMBDA_m * mask_recon_loss \
        ...
        add_moving_summary(G_loss_img, D_loss_img, FM_loss_mask, mask_recon_loss,
