...
"""
Model b:
The density field is represented by MLPs.
"""
...
DIS_SCALE = 3
SHAPE = 256
REZ = 32  # resolution of feature images
NF = 64  # channel size
M_F = 32  # feature size
...
class Model(GANModelDesc):
    ...
    def get_densities(self, input, nb_layers=8, hidden_d=128, name='get_densities'):
    ...
    def volume_renderer(self,
        ...
        assert len(zs) == 2, "z should be [z_p, z_f]"
        z_p = zs[0]
        z_f = zs[1]
        ...
        points = tf.tile(points, (tf.shape(z_p)[0], 1, 1, 1))
        directions = tf.tile(directions, (tf.shape(z_p)[0], 1, points.shape[-2], 1))
        z_p = tf.tile(z_p, (1, points.shape[1], points.shape[2], 1))
        ...
        d_input = tf.concat([z_p, points], axis=-1)
        densities = self.get_densities(d_input)

        # stack z_f, points, directions together
        f_input = tf.concat([z_f, points, directions], axis=-1)

        # pass into the MLPs to get feature vectors
        features = self.get_features(f_input, output_d=feat_dim)
        ...
        # get density maps
        density_map = numerical_interg(sigmas=densities, fs=None, include_feature=False)
        density_map = tf.reshape(density_map, [-1, rez, rez, 1])
        for _ in range(int(np.log2(SHAPE // rez))):
            density_map = UpSampling2D(interpolation='bilinear')(density_map)

        density_map = tf.transpose(density_map, (0, 2, 1, 3))

        return feature_maps, density_map
    ...
    def build_graph(self, img, box, mask, bbx, z, z2):
        ...
        with argscope([Conv2D, Conv2DTranspose, tf.layers.conv2d]):
            ...
            with tf.variable_scope('gen'):
                ...
                with tf.variable_scope('gen_img'):
                    feature_img, density_map = self.volume_renderer([z_p, z_f], M_S=M_S)
                    ...
                    # print(gen_img)
                    # density_map = self.volume_renderer(z_f,
                    #                                    M_S=M_S,
                    #                                    rez=16, include_feature=False)
                    density_map = tf.identity(density_map, name='density_map')
                    # print(density_map)
                    ...
                    # print(mus_rot)
                    # print(sigmas_rot)
                    feature_img_rot, density_map_rot = self.volume_renderer([z_p, z_f], roty=randomrot,
                                                           M_S=M_S)
                    ...
                    # density_map_rot = self.volume_renderer(z_f,
                    #                                        M_S=M_S,
                    #                                        roty=randomrot, rez=16, include_feature=False)
                    density_map_rot = tf.identity(density_map_rot, name='density_map_rot')
                ...
                with tf.variable_scope('gen_img'):
                    feature_img_cyc, density_map_cyc = self.volume_renderer([z_p_rot, z_f_rot], roty=-1.*randomrot,
                                                           M_S=M_S)
        ...
        add_moving_summary(G_loss_img, D_loss_img, FM_loss_mask,
                           ...
                           mask_recon_cycle_loss, img_recon_cycle_loss)
