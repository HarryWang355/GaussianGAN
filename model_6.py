...
1. density map is turned into masks (per gaussian)
"""
...
def density2mask(density):
    """
    An aggressively steep sigmoid function that turns density values to approximately binary masks.

    :param img: input img with RGB values scaled to [0, 1].
    :return: a binary mask
    """
    m = 1/(1+tf.exp(-200*(density-0.1)))
        ...
        with argscope([Conv2D, Conv2DTranspose, tf.layers.conv2d]):
            ...
            with tf.variable_scope('gen'):
                ...
                with tf.variable_scope('gen_img'):
                    ...
                    density_map = density2mask(density_map)
                    ...
                    density_map_rot = density2mask(density_map_rot)
        ...
        with tf.name_scope('losses'):
            with tf.name_scope('mask_losses'):
                ...
                with tf.name_scope('density_loss'):
                    density_loss = tf.reduce_mean(tf.abs(mask - density_map), name='density_loss')
                    rot_density_loss = tf.reduce_mean(tf.abs(gen_mask_rot - density_map_rot),
