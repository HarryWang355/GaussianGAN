...
class Model(GANModelDesc):
    ...
    def volume_renderer(self,
        ...
        z = tf.tile(z, (1, points.shape[1], points.shape[2], 1))
        ...
        d_input = tf.concat([z, points], axis=-1)
        densities = self.get_densities(d_input)

        if include_feature:

            # stack z_f, points, directions together
            net_input = tf.concat([z, points, directions], axis=-1)
        ...
        with argscope([Conv2D, Conv2DTranspose, tf.layers.conv2d]):
            ...
            with tf.variable_scope('gen'):
                ...
                with tf.variable_scope('gen_img'):
                    feature_img = self.volume_renderer(z_f, M_S=M_S)
                    ...
                    # print(mus_rot)
                    # print(sigmas_rot)
                    feature_img_rot = self.volume_renderer(z_f, roty=randomrot,
                                                           M_S=M_S)
                ...
                with tf.variable_scope('gen_img'):
                    feature_img_cyc = self.volume_renderer(z_f_rot, roty=-1.*randomrot,
                                                           M_S=M_S)
