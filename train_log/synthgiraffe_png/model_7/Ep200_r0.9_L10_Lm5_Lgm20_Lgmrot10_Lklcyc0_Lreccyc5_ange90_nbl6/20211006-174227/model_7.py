...
"""
Previous changes from model5_2:
1. img2mask becomes differentiable
2. img_recon_loss is removed
3. recon_cycle_loss becomes a l1 loss between mask and gen_mask_cyc
4. img_recon_loss is added back, but it should have small weight
5. IoU loss is used in place of l1 for mask reconstruction
6. resolution of feature/density map increased to 32*32
7. fixed bug about rotating twice
...
DIS_SCALE = 3
SHAPE = 256
REZ = 32  # feature map resolution
NF = 64  # channel size
M_F = 32  # feature size
M_S = 32  # number of points sampled along each ray
Lx = 30
Ld = 12
N_DIS = 4
N_SAMPLE = 2
...
def point_sampling(rez=REZ, M_S=M_S, ray_len=2., f=1.):
    """
...
def img2mask(img):
    """
    An aggressively steep sigmoid function that turns rgb values to approximately binary values.

    :param img: input img with RGB values scaled to [-1, 1].
    :return: a binary mask
    """
    m = tf.reduce_mean(img, axis=-1)
    m = tf.expand_dims(m, axis=-1)
    m = 1/(1+tf.exp(-200*(m+0.95)))
...
class Model(GANModelDesc):
    ...
    def encoder(self, img, mask):
        ...
        with argscope([Conv2D, Conv2DTranspose], activation=INLReLU):
            x1 = Conv2D('mask_conv0_0', img, NF, 7, strides=2, activation=INLReLU)
    ...
    def volume_renderer(self,
        ...
        points = tf.tile(points, (tf.shape(mus)[0], 1, 1, 1))
        directions = tf.tile(directions, (tf.shape(mus)[0], 1, points.shape[-2], 1))
        ...
        if include_feature:
            ...
            # net_input = tf.concat([points, directions], axis=-1)

            # pass into the MLPs to get feature vectors
            features = self.get_features(tf.concat([points, directions], axis=-1), output_d=feat_dim)
    ...
    def neural_renderer(self, f_img, input_dim=128, output_dim=3, min_feat=32, img_size=256):
        ...
        nb_blocks = int(math.log2(img_size / REZ))
    ...
    def build_graph(self, img, box, mask, bbx, z, z2):
        ...
        with argscope([Conv2D, Conv2DTranspose, tf.layers.conv2d]):
            ...
            with tf.variable_scope('gen'):
                ...
                with tf.variable_scope('gen_img'):
                    feature_img = self.volume_renderer(mus_org, sigmas_org, M_S=M_S)
                    ...
                    # print(gen_img)
                    density_map = self.volume_renderer(mus_org, sigmas_org,
                                                       M_S=M_S,
                                                       rez=REZ, include_feature=False)
                    ...
                    # print(mus_rot)
                    # print(sigmas_rot)
                    feature_img_rot = self.volume_renderer(mus_org, sigmas_org, roty=randomrot,
                                                           M_S=M_S)
                    feature_img_rot = tf.identity(feature_img_rot, name='feature_img_rot')
                    ...
                    density_map_rot = self.volume_renderer(mus_org, sigmas_org,
                                                           M_S=M_S,
                                                           roty=randomrot, rez=REZ, include_feature=False)
                ...
                ##cycle
                with tf.variable_scope('foreground_encoder'):
                    z_p_rot = self.encoder(gen_img_rot, gen_mask_rot)
                ...
                with tf.variable_scope('gen_gaussians'):
                    ...
                    mus_cyc, sigmas_cyc = apply_rotation(mus_cyc, sigmas_cyc, zrs, thets_cyc, zrs)
                    # mus_cyc, sigmas_cyc = apply_rotation(mus_cyc, sigmas_cyc, zrs, thets_cyc - randomrot, zrs)
                with tf.variable_scope('gen_img'):
                    feature_img_cyc = self.volume_renderer(mus_cyc, sigmas_cyc, roty=-1.*randomrot,
                                                           M_S=M_S)
                    gen_img_cyc = self.neural_renderer(feature_img_cyc, input_dim=M_F)
                    gen_img_cyc = gen_img_cyc * 2.0 - 1.
                    gen_mask_cyc = img2mask(gen_img_cyc)
        ...
        with tf.name_scope('losses'):
            with tf.name_scope('mask_losses'):
                ...
                with tf.name_scope('cycle_loss'):
                    ...
                    img_recon_cycle_loss = tf.reduce_mean(tf.abs(img - gen_img_cyc), name='img_recon_cycle_loss')
                    # img_recon_cycle_loss = tf.reduce_mean(tf.abs(img - gen_img_cyc), name='img_recon_cycle_loss')
                    mask_recon_cycle_loss = tf.subtract(1., IoU_loss(mask, gen_mask_cyc), name='mask_recon_cycle_loss')
                    # gm_cycle_loss = tf.reduce_mean(tf.abs(pose_embeddings[-1] - pose_embeddings_cyc[-1]),
                    #                                name='cycle_loss')

        self.g_loss = G_loss_img + LAMBDA * FM_loss_mask + LAMBDA_mask_recon * mask_recon_loss \
                      + LAMBDA_m * img_recon_loss \
                      + LAMBDA_kl_cyc * kl_cycle_loss \
                      + LAMBDA_mask_recon_cyc * mask_recon_cycle_loss \
                      + LAMBDA_img_recon_cyc * img_recon_cycle_loss\
        ...
        add_moving_summary(G_loss_img, D_loss_img, FM_loss_mask,
                           ...
                           kl_cycle_loss, mask_recon_cycle_loss, img_recon_cycle_loss)
...
def export_compact(model_path):
    ...
    pred_config = PredictConfig(
        ...
        output_names=['gen/gen_img/gen_img_rot',
                      'gen/gen_img/density_map_rot',
                      'gen/gen_img/feature_img_rot'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--LAMBDA_mask_recon', help='hyperparameter', default=50.0, type=float)
    ...
    parser.add_argument('--LAMBDA_mask_recon_cyc', help='hyperparameter', default=5.0, type=float)
    parser.add_argument('--LAMBDA_img_recon_cyc', help='hyperparameter', default=5.0, type=float)
    ...
    LAMBDA_mask_recon_cyc = args.LAMBDA_mask_recon_cyc
    LAMBDA_img_recon_cyc = args.LAMBDA_img_recon_cyc
    LAMBDA_mask_recon = args.LAMBDA_mask_recon
    ratio = args.ratio
    angle = args.angle
    BATCH = args.batch
    ...
    logdir = os.path.join('train_log', args.dataset, basename,
                          'Ep%d_r%.1f_L%d_Lm%d_Lgm%d_Lgmrot%d_Lklcyc%d_Lreccyc%d_ange%d_nbl%d' % (nb_epochs, ratio, LAMBDA,
                                                                                                  ...
                                                                                                  LAMBDA_dens_rot,
                                                                                                  LAMBDA_kl_cyc, LAMBDA_mask_recon_cyc,
                                                                                                  angle, nb_landmarks),
